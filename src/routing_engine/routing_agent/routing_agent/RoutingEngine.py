...
class RoutingEngine:
    ...
    def solve(self):
        if(len(self.taskList)>1):
            ...
            self.taskSequence=[self.fleet[0].locationNode.id,self.taskList[0].locationNode.id]
        else:
    ...
    def findPathBetweenTwoPoints(self,fromnodeid:str,tonodeid:str):
        ...
        #print(idPath)
        print(f"[DEBUG] Path from {fromnodeid} to {tonodeid} => {idPath}")
    ...
    def update(self,currentnodeid):
        ...
        if len(self.latestSolutionPath) > 0:
            ...
            with open("src/env_config.json", 'r', encoding='utf-8') as f:
                ...
                env_config["latestSolutionPath"] = self.latestSolutionPath
            with open("src/env_config.json", 'w', encoding='utf-8') as f:
            ...
            if currentnodeid in self.latestSolutionPath:
                ...
                if self.latestSolutionPath[0] in self.waypointGraph.getNodeById(currentnodeid).edges:
                    print("Rebuild path")
                else:
                    raise KeyError(f"You derail from the path!{currentnodeid} not in {self.latestSolutionPath}")
        else:
            return []
